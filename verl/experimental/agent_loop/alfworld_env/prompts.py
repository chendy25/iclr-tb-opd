...
``iclr/logs/_backup_alfworld_nothink_20260831/``. The real-thinking protocol
(``enable_thinking=True``) lives under experiment names ending in ``_thinking``.
"""
...
# Same answer contract as ATOD's per-turn prompt. Every env step appends one of
# these to the response; with enable_thinking=False the model is not generating
# a real think block, so the extra tokens are cheap relative to observations.
